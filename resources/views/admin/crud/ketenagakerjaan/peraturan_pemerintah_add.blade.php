...
@include('layouts.admin.sidebar')
  <div class="content-wrapper">
    <div class="col-12 grid-margin stretch-card">
      <div class="card">
        <div class="card-body">
          <h4 class="card-title">Peraturan Pemerintah</h4><hr>
            @if(session('status'))
              <div class="alert alert-success" style="background-color: #1bcfb4">
                <p style="color: white">{{ session('status') }}</p>
              </div>
            @endif
          <form class="forms-sample" method="POST" action="{{ route('peraturan_pemerintah_store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <a href="{{ route('peraturan_pemerintah') }}" class="btn btn-light">Halaman Data Peraturan Pemerintah</a>
          </form>
        </div>
      </div>
    </div>
